...
use api::{DevicePixelScale, RasterRect, RasterSpace, DeviceIntPoint, ColorF, ImageKey, DirtyRect, WorldSize};
...
impl TileCache {
    ...
    pub fn update_prim_dependencies(
        ...
        // !!!!!!!!!!!!!! use world space for above too (remove map_local_to_pic)
        let prim_world_rect = self.map_local_to_world.map(&culling_rect).expect("no world rect!");

        // Get the tile coordinates in the picture space.
        let (p0, p1) = self.get_tile_coords_for_rect(&rect);

        println!("rect {:?} has tile_coords {:?} {:?}", rect, p0, p1);
        ...
        for y in p0.y - self.tile_origin.y .. p1.y - self.tile_origin.y {
            for x in p0.x - self.tile_origin.x .. p1.x - self.tile_origin.x {
                ...
                let origin = WorldPoint::new(
                    prim_world_rect.origin.x - tile.world_rect.origin.x,
                    prim_world_rect.origin.y - tile.world_rect.origin.y
                );
...
impl<T> ComparableVec<T> where T: PartialEq + Clone {
    fn new() -> Self {
        ComparableVec {
            items: Vec::new(),
            current_index: 0,
